...
void GameStateManager::goToStory()
{
	// IF THE PLAYER IS IN THE FIRST STORY SCREEN, MOVE TO THE NEXT PART OF THE STORY
	if (currentGameState == GS_MENU_STORY_1_MENU)
		currentGameState = GS_MENU_STORY_2_MENU;
	// IF THE PLAYER IS IN THE SECOND STORY SCREEN, 
	// MOVE BACK TO THE FIRST PART OF THE STORY.
	// THE LOGIC IS POSSIBLE BECAUSE WE ONLY HAVE 2
	// SCREENS. IF WE HAD MORE, SEPARATE FUNCTIONS 
	// MUST BE MADE.
	else
		currentGameState = GS_MENU_STORY_1_MENU;
}

void GameStateManager::goToCredits()
{
	currentGameState = GS_CREDITS;
}

void GameStateManager::goToPrompt()
{
	if (currentGameState == GS_MAIN_MENU)
		currentGameState = GS_PROMPT;
	else if (currentGameState == GS_PAUSED)
		currentGameState = GS_IN_GAME_PROMPT;
}
...
/*
	This will put the current game state into PAUSE iff the game is currently
	in progress, and vice-versa.
*/
void GameStateManager::goToPause()
{
...
void GameStateManager::goToLevelChooser()
{
	currentGameState = GS_LEVEL_MENU;
}
...
/*
	Used to test if the game is currently paused.
*/
...
void GameStateManager::update(Game *game)
{
	...
	if (game->getGSM()->isGameInProgress())	// I don't think this check is necessary
	{
		for (list<GameObject*>::iterator it = GameObject::activeGameObjects.begin(); it != GameObject::activeGameObjects.end(); it++)
		{
			(*it)->update();
		}
