...
/* 
    DSP callbacks (FMOD Studio new functionality)
*/
...
typedef FMOD_RESULT (F_CALLBACK *FMOD_DSP_SHOULDIPROCESS)          (FMOD_DSP_STATE *dsp_state, bool inputsidle, unsigned int length, int outchannels, int inchannels, FMOD_SPEAKERMODE speakermode);
...
typedef enum
{
    FMOD_DSP_PARAMETER_UIHINT_NONE = 0,
    FMOD_DSP_PARAMETER_UIHINT_SLIDER,
    FMOD_DSP_PARAMETER_UIHINT_RADIOBUTTON,
    FMOD_DSP_PARAMETER_UIHINT_CHECKBOX,
    FMOD_DSP_PARAMETER_UIHINT_TEXTEDIT,
    FMOD_DSP_PARAMETER_UIHINT_FORCEINT = 65536    /* Makes sure this enum is signed 32bit. */
} FMOD_DSP_PARAMETER_UIHINT;
...
typedef struct FMOD_DSP_PARAMETER_DESC
{
    ...
    FMOD_DSP_PARAMETER_UIHINT uihint;             /* [w] Hint to UI writers about how this parameter should ideally be presented to the user. */
    char                      name[16];           /* [w] Name of the parameter to be displayed (ie "Cutoff frequency"). */
    char                      label[16];          /* [w] Short string to be put next to value to denote the unit type (ie "hz"). */
    const char               *description;        /* [w] Description of the parameter to be displayed as a help item / tooltip for this parameter. */
} FMOD_DSP_PARAMETER_DESC;
...
*/
typedef struct FMOD_DSP_DESCRIPTION2
{
    char                             name[32];           /* [w] Name of the unit to be displayed in the network. */
    unsigned int                     version;            /* [w] Plugin writer's version number. */
    int                              channels;           /* [w] Number of channels.  Use 0 to process whatever number of channels is currently in the network.  >0 would be mostly used if the unit is a unit that only generates sound. */
...
typedef enum
{
    FMOD_DSP_THREE_EQ_LOWGAIN,       /* Low frequency gain in dB.  -80.0f to 10.0f.  Default = 0. */
    FMOD_DSP_THREE_EQ_MIDGAIN,       /* Mid frequency gain in dB.  -80.0f to 10.0f.  Default = 0. */
    FMOD_DSP_THREE_EQ_HIGHGAIN,      /* High frequency gain in dB.  -80.0f to 10.0f.  Default = 0. */
    FMOD_DSP_THREE_EQ_LOWCROSSOVER,  /* Low-to-mid crossover frequency in Hz.  10.0 to 22000.0.  Default = 400.0. */
    FMOD_DSP_THREE_EQ_HIGHCROSSOVER, /* Mid-to-high crossover frequency in Hz.  10.0 to 22000.0.  Default = 4000.0. */
    FMOD_DSP_THREE_EQ_CROSSOVERSLOPE /* Crossover Slope.  0 = 12dB/Octave, 1 = 24dB/Octave, 2 = 48dB/Octave.  Default = 1 (24dB/Octave). */
} FMOD_DSP_THREE_EQ;
...
typedef enum
{
    ...
    FMOD_DSP_ECHO_FEEDBACK,    /* Echo decay per delay.  0 to 100.  100.0 = No decay, 0.0 = total decay (ie simple 1 line delay).  Default = 50.0. */
    FMOD_DSP_ECHO_DRYLEVEL,      /* Original sound volume in dB.  -80.0f to 10.0f.  Default = 0. */
    FMOD_DSP_ECHO_WETLEVEL       /* Volume of echo signal to pass to output in dB.  -80.0f to 10.0f.  Default = 0. */
} FMOD_DSP_ECHO;
...
typedef enum
{
    FMOD_DSP_FLANGE_MIX,         /* Percentage of wet signal in mix.  0 to 100. Default = 50. */
...
typedef enum
{
    FMOD_DSP_LIMITER_RELEASETIME,   /* Time to ramp the silence to full in ms.  1.0 to 1000.0. Default = 10.0. */
    FMOD_DSP_LIMITER_CEILING,       /* Maximum level of the output signal in dB.  -12.0 to 0.0.  Default = 0.0. */
    FMOD_DSP_LIMITER_MAXIMIZERGAIN, /* Maximum amplification allowed in dB.  0.0 to 12.0.  Default = 0.0. 0.0 = no amplifaction, higher values allow more boost. */
    FMOD_DSP_LIMITER_MODE,          /* Channel processing mode. 0 or 1. Default = 0. 0 = Independent (limiter per channel), 1 = Linked*/
} FMOD_DSP_LIMITER;
...
typedef enum
{
    ...
    FMOD_DSP_PARAMEQ_GAIN        /* Frequency Gain in dB.  -30 to 30.  Default = 0.  */
} FMOD_DSP_PARAMEQ;
...
typedef enum
{
    FMOD_DSP_CHORUS_MIX,      /* Volume of original signal to pass to output.  0.0 to 100.0. Default = 50.0. */
    FMOD_DSP_CHORUS_RATE,     /* Chorus modulation rate in Hz.  0.0 to 20.0.  Default = 0.8 Hz. */
    FMOD_DSP_CHORUS_DEPTH,    /* Chorus modulation depth.  0.0 to 100.0.  Default = 3.0. */
} FMOD_DSP_CHORUS;
...
typedef enum
{
    FMOD_DSP_COMPRESSOR_THRESHOLD,   /* Threshold level (dB) in the range from -80 through 0. The default value is 0. */ 
    FMOD_DSP_COMPRESSOR_RATIO,       /* Compression Ratio (dB/dB) in the range from 1 to 50. The default value is 2.5. */ 
    FMOD_DSP_COMPRESSOR_ATTACK,      /* Attack time (milliseconds), in the range from 10 through 1000. The default value is 20. */
    FMOD_DSP_COMPRESSOR_RELEASE,     /* Release time (milliseconds), in the range from 10 through 5000. The default value is 100 */
    ...
    FMOD_DSP_COMPRESSOR_USESIDECHAIN /* [bool] Whether to analyse the sidechain signal instead of the input signal. The default value is false */
} FMOD_DSP_COMPRESSOR;
...
typedef enum
{
    FMOD_DSP_SFXREVERB_DECAYTIME,           /* Decay Time       : Reverberation decay time at low-frequencies in milliseconds.  Ranges from 100.0 to 20000.0. Default is 1500. */
    FMOD_DSP_SFXREVERB_EARLYDELAY,          /* Early Delay      : Delay time of first reflection in milliseconds.  Ranges from 0.0 to 300.0.  Default is 20. */
    FMOD_DSP_SFXREVERB_LATEDELAY,           /* Reverb Delay     : Late reverberation delay time relative to first reflection in milliseconds.  Ranges from 0.0 to 100.0.  Default is 40. */
    FMOD_DSP_SFXREVERB_HFREFERENCE,         /* HF Reference     : Reference frequency for high-frequency decay in Hz.  Ranges from 20.0 to 20000.0. Default is 5000. */
    FMOD_DSP_SFXREVERB_HFDECAYRATIO,        /* Decay HF Ratio   : High-frequency decay time relative to decay time in percent.  Ranges from 10.0 to 100.0. Default is 50. */
    FMOD_DSP_SFXREVERB_DIFFUSION,           /* Diffusion        : Reverberation diffusion (echo density) in percent.  Ranges from 0.0 to 100.0.  Default is 100. */
    FMOD_DSP_SFXREVERB_DENSITY,             /* Density          : Reverberation density (modal density) in percent.  Ranges from 0.0 to 100.0.  Default is 100. */
    FMOD_DSP_SFXREVERB_LOWSHELFFREQUENCY,   /* Low Shelf Frequency : Transition frequency of low-shelf filter in Hz.  Ranges from 20.0 to 1000.0. Default is 250. */
    FMOD_DSP_SFXREVERB_LOWSHELFGAIN,        /* Low Shelf Gain   : Gain of low-shelf filter in dB.  Ranges from -36.0 to 12.0.  Default is 0. */
    FMOD_DSP_SFXREVERB_HIGHCUT,             /* High Cut         : Cutoff frequency of low-pass filter in Hz.  Ranges from 20.0 to 20000.0. Default is 20000. */
    FMOD_DSP_SFXREVERB_EARLYLATEMIX,        /* Early/Late Mix   : Blend ratio of late reverb to early reflections in percent.  Ranges from 0.0 to 100.0.  Default is 50. */
    FMOD_DSP_SFXREVERB_WETLEVEL,            /* Wet Level        : Reverb signal level in dB.  Ranges from -80.0 to 20.0.  Default is -6. */
    FMOD_DSP_SFXREVERB_DRYLEVEL             /* Dry Level        : Dry signal level in dB.  Ranges from -80.0 to 20.0.  Default is 0. */
} FMOD_DSP_SFXREVERB;
...
typedef enum
{
    FMOD_DSP_SEND_RETURNID,     /* ID of the Return DSP this send is connected to (integer values only). -1 indicates no connected Return DSP. Default = -1. */
    FMOD_DSP_SEND_LEVEL,        /* Send level. 0.0 to 1.0. Default = 1.0 */
    FMOD_DSP_SEND_INPUT_FORMAT, /* Format of the input signal. */
} FMOD_DSP_SEND;
...
typedef enum
{
    FMOD_DSP_PAN_INPUT_FORMAT,
    FMOD_DSP_PAN_OUTPUT_FORMAT,
    FMOD_DSP_PAN_STEREO_POSITION,
    FMOD_DSP_PAN_SURROUND_DIRECTION,
    FMOD_DSP_PAN_SURROUND_EXTENT,
    FMOD_DSP_PAN_SURROUND_ROTATION,
    FMOD_DSP_PAN_SURROUND_LFE_LEVEL,
    FMOD_DSP_PAN_SURROUND_FROM_STEREO_MODE,
    FMOD_DSP_PAN_SURROUND_STEREO_SEPARATION,
    FMOD_DSP_PAN_SURROUND_STEREO_AXIS,
    FMOD_DSP_PAN_ENABLED_SURROUND_SPEAKERS,
    FMOD_DSP_PAN_3D_ANGLE,
    FMOD_DSP_PAN_3D_DISTANCE,
    FMOD_DSP_PAN_3D_ORIENTATION,
...
typedef enum
{
    FMOD_DSP_RETURN_ID,             /* ID of this Return DSP (integer values only). Read-only. */
    FMOD_DSP_RETURN_INPUT_FORMAT,   /* Input format of this return. */
} FMOD_DSP_RETURN;
