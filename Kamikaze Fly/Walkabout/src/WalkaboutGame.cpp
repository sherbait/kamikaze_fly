...
#include "src\WalkaboutMouseEventHandler.h"
...
{
	// CREATE THE GAME
	Game *walkaboutGame = Game::getInstance();
	...
	WalkaboutMouseEventHandler* mouseHandler = new WalkaboutMouseEventHandler();
	walkaboutGame->getInput()->registerMouseHandler(mouseHandler);

	// THIS WILL HANDLE PHYSICS COLLISION EVENTS
	KamikazeFlyCollisionListener *kamikazeFlyCollisionListener = new KamikazeFlyCollisionListener(walkaboutGame);
	walkaboutGame->getGSM()->getPhysics()->setCollisionListener(kamikazeFlyCollisionListener);

	// This will handle audio appropriation at any given time in game.
	KamikazeFlyAudioHandler *kamikazeFlyAudioHandler = new KamikazeFlyAudioHandler();
	walkaboutGame->getAudioManager()->registerAudioHandler((KamikazeFlyAudioHandler*)kamikazeFlyAudioHandler);
